...
public:
    ...
    float length()
    {
        ...
        for (int i = 0; i < SIZE; i++)
            result += pow(m_atData[i], 2);
        result = sqrt(result);
        return abs(result);
    }

    CVector<T, SIZE> square()
    {
        float result[SIZE];
        for (int i = 0; i < SIZE; i++)
            result[i] = pow(m_atData[i], 2);
        CVector<T, SIZE> res;
        res.setData(result);
        return res;
    }

    CVector<T, SIZE> squareRoot()
    {
        float result[SIZE];
        for (int i = 0; i < SIZE; i++)
            result[i] = sqrt(m_atData[i]);
        CVector<T, SIZE> res;
        res.setData(result);
        return res;
    }

    CVector<T, SIZE> normalize()
    {
        float length;
        for (int i = 0; i < SIZE; i++)
            length += pow(m_atData[i], 2);
        length = abs(sqrt(length));

        float result[SIZE];
        for (int i = 0; i < SIZE; i++)
            result[i] = m_atData[i] / length;
        CVector<T, SIZE> res;
        res.setData(result);
        return res;
    }
    ...
    CVector<T, SIZE> operator / (const CVector<T, SIZE> & vec) 
	{
        ...
        float res[SIZE];
		for (int i=0; i<SIZE; i++) res[i] = m_atData[i] / vec.m_atData[i];
        result.setData(res);
		return result;
	}
